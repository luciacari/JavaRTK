...
public class Game {
    ...
    private void registerCommands() {
        ...
        commands.put("about", (ctx, a) -> {
            ...
            System.out.println("Доступные команды: help, about, gc-stats, look, move, take, inventory, use, fight, save, load, exit.");
        });
        ...
        commands.put("move", (ctx, a) -> {
            if (a.isEmpty()) throw new InvalidCommandException("Для команды 'move': необходимо указать направление (north/south/east/west)");
            String dir = a.get(0).toLowerCase(Locale.ROOT);
            Room cur = ctx.getCurrent();
            Room next = cur.getNeighbors().get(dir);

            if (next == null) throw new InvalidCommandException("Из этой комнаты нельзя идти в направлении " + dir + ".");
        ...
        commands.put("exit", (ctx, a) -> {
            System.out.println("До новой встречи! Ваш результат сохранен.");
            System.exit(0);
        });
    ...
    public void run() {
        System.out.println("Добро пожаловать в игру DungeonMini! Введите 'help' для просмотра доступных команд.");
